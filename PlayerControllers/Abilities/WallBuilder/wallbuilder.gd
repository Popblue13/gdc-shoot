...
func gloot(data: Dictionary) -> Node:
	...
	var turret_scene = load("res://PlayerControllers/Abilities/Wall/wall.tscn").instantiate()
	turret_scene.position = data["pos"]
	turret_scene.rotation = data["rot"]
